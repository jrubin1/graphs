...
class Rule {
    ...
    void addDependency(Rule dependent) {
        int v = getVertex();
        int otherV = dependent.getVertex();
        _depends.add(v, otherV);
    }
    ...
    private void checkFinishedDependencies() {
        for (int continuations : _depends.successors(_vertex)) {
            boolean unfinished =
                    _depends.getLabel(continuations).isUnfinished();
            if (unfinished) {
                error("Dependences are not yet built");
            }
        }
    }
    ...
    private boolean outOfDate() {
        if (_time == null) {
            ...
            for (int successors : _depends.successors(_vertex)) {
                int dependsTime = _depends.getLabel(successors).getTime();
                if (dependsTime > _time) {
                    return true;
                }
            }
        }
    ...
    void rebuild() {
        ...
        if (outOfDate()) {
            ...
            int temp = _maker.getCurrentTime();
            _time = temp;
        }
        _finished = true;
    }
